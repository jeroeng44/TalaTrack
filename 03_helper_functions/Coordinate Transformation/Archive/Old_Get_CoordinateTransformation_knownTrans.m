function [R, t] = Get_CoordinateTransformation_fromFiducials(Points_A, Points_B)
    %% INPUT:
    ...
    [R, ~] = Get_CoordinateTransformation_simple(Points_A - t_prime, Points_B);
    t = -R * t_prime;
end

function [R, t] = Get_CoordinateTransformation_simple(Points_A, Points_B)
    %% INPUT:
    % Points_A: A 3xN matrix where each column represents a 3D point in system A.
    % Points_B: A 3xN matrix where each column represents a 3D point in system B.
    %% OUTPUT:
    ...
    cond_number = cond(covariance_matrix);
    disp('Cond number')
    disp(cond_number);
    pause
